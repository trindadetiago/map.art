...
Two packages ship React, both with React as a peer dep, each justified by 2+ consumers: `@mapart/renderer` (`<Scene>`, shared by `apps/web` and `apps/worker-render/render-page`, the page headless Chrome navigates to) and `@mapart/ui` (shared client components — the globe, the worldmap, the analytics provider). Tree-shaking + per-component subpaths keep non-React consumers from pulling component code at runtime. New packages follow the "no React" default unless there's a similar two-or-more-consumer justification — and a new *shared component* belongs under `@mapart/ui`, not its own package.

### `apps/`

| Path | What | Runtime shape |
|---|---|---|
| `apps/web` | Next.js 15 (App Router). Main UI on `:3210`. Admin pages at `/admin/*` (projects, storage, env, models, renderer, tiles, stylize, analytics). Database has no panel — its card on the dashboard links to Drizzle Studio at https://local.drizzle.studio. Analytics has no panel of its own either — it iframes a shared PostHog dashboard (`POSTHOG_DASHBOARD_TOKEN`), since PostHog only permits framing its shared URLs. | long-running service |
| `apps/visualizer` | Next.js 15 (App Router). Deep-zoom viewer on `:3220`. One OpenSeadragon instance over a project's pre-built DZI/WebP pyramid, routed at `/<slug>` (a project id still resolves and redirects to its slug); pixel-art kept crisp (smoothing off, nearest-neighbour over-zoom). Tiles come straight from the published bucket's own origin when `VIZ_PUBLIC_BASE_URL` is set (a CDN answers and no app process touches the bytes); `app/api/viz/[...path]` is the fallback for when it isn't, proxying the same objects out of `@mapart/storage`. Both addresses are built by one helper, `vizObjectUrl` in `lib/project.ts`, so the deep-zoom tiles and the landing thumbnails always agree. The runtime is a flat raster, no Three.js/scene. Lat/lng pins (`viz/{projectId}/pins.json`) are rendered as OSD overlays, mapped to image pixels via the pyramid's `VizGeoAnchor` (`lib/geo.ts`). Pyramids are built offline by `@mapart/export` (via `mapart export dzi`). Reads no database: `viz/index.json` — the catalogue the export writes — is what it resolves slugs and globe pins against, so the public site keeps serving whether or not the pipeline is up. Deployed on Vercel rather than Railway, which is why it must stay database-free. | serverless |
...
| `@mapart/ui` | Shared React components, one subpath per component. `@mapart/ui/globe` (pure helpers — `buildGlobeTexture`, `getCountryFeatures`, `GLOBE_VARIANTS` — via d3-geo + topojson + world-atlas) and `@mapart/ui/globe/react` (the `<Globe>` component, `'use client'`); `@mapart/ui/worldmap` + `@mapart/ui/worldmap/react` follow the same split. `@mapart/ui/analytics` is `<AnalyticsProvider>` — PostHog for both Next apps, taking the project token as a prop so the host app's server layout reads it from `@mapart/env` (no `NEXT_PUBLIC_` inlining, so the token is a runtime value). It points posthog-js at a same-origin `/ingest`, which each app's `next.config.ts` rewrites to PostHog; empty token means analytics stays off. Consumed by `apps/web` (home + UI catalog) and `apps/visualizer` (globe home builds its own pin layer on the pure helpers). React + three are peer/regular deps like `@mapart/renderer`. |
| `@mapart/storage` | Blob storage on S3 (MinIO locally, Railway Buckets in prod), configured by the `S3_*` env vars. Singleton via `getStorage()`. A second target, `getVizStorage()` on `VIZ_S3_*`, holds everything under `viz/` — the only objects served publicly, so they sit in their own world-readable bucket (Cloudflare R2, behind `tiles.earthtopixels.com`) that the pipeline's credentials can't reach. With `VIZ_S3_BUCKET` unset there is no split and it resolves to `getStorage()`, which is what local dev wants. `put()` takes optional `contentType`/`cacheControl` — objects served straight from a bucket have no app in the path to add headers. |
...
| `@mapart/export` | Builds a project's deep-zoom pyramid for the visualizer. `exportProjectDzi(projectId, opts)` stitches the per-tile `(x,y)` images (stylized by default, or rendered) into one raster, slices a DZI/WebP pyramid via sharp/libvips, and uploads it to the published bucket under `viz/{projectId}/v/{version}/` (`tiles.dzi` + `tiles_files/`), where `version` identifies that export. `viz/{projectId}/metadata.json` — a `VizMetadata`, written last and uncached — names the current version and is what makes the switch atomic: until it lands the previous pyramid is still live. Versioning is what lets tiles be served `immutable`; overwriting a fixed key left stale copies in browser and CDN caches that a rebuild could not displace. Old versions are pruned to the last two. The `metadata.json` also carries a `VizGeoAnchor` — a linear fit from grid coords to WGS84 over the tile centres — so the visualizer can place lat/lng pins. Pins live alongside as `pins.json` (a `VizPin[]`); `getProjectPins`/`setProjectPins`/`parsePins` (`./pins`) read, write, and validate them. Sub-paths: `./keys` (storage-key layout), `./pins` (pin I/O), `./geo` (pure grid↔WGS84 fit — `computeGeoAnchor`, `gridOffsetToLatLng`, `latLngToGridOffset`), `./types` (`VizMetadata`, `VizGeoAnchor`, `VizPin`). Depends on `@mapart/db`, `@mapart/storage`, `@mapart/logger`. |
...
pnpm mapart export run-queued          # claim the oldest queued export and run it (what export-runner does)
