...
import { closeDb, getSql } from '../src/client';
import { resetSchema, runMigrations } from '../src/migrate';
import { listModels, seedDefaultModels, upsertModel } from '../src/repos/models';
import { createRectProject, deleteProject, listProjects } from '../src/repos/projects';
import { getPostgresInfo, getTableCounts } from '../src/repos/stats';
import { countTilesForProject } from '../src/repos/tiles';

const program = new Command();
program.name('mapart-db').description('Manage the application database');
...
const projectsCmd = program.command('projects').description('Project CRUD');
...
projectsCmd
  ...
  .action(async (opts) => {
    try {
      const { project, tileCount } = await createRectProject({
        name: opts.name,
        slug: opts.slug,
        centerLat: opts.lat,
        centerLng: opts.lng,
...
projectsCmd
  ...
  .action(async (opts) => {
    try {
      const n = await countTilesForProject(opts.id);
      console.log(n);
    } finally {
      await closeDb();
    }
...
const modelsCmd = program.command('models').description('Model registry');
...
program
  ...
  .action(async () => {
    try {
      const n = await seedDefaultModels();
